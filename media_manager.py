...
"""
==============================================================================
MEDIA MANAGER (Gestor de Recursos Visuales y Sonoros - VERSIÓN SHORTS)
==============================================================================
...
Adaptado para el rastreo y logging de la pipeline de Shorts.
"""
...
def get_random_file_from_dir(directory):
    ...
    if not os.path.exists(directory):
        logger.warning(f"  [MediaManager Short] Directorio no encontrado: {directory}")
        return None
    ...
    if not files:
        logger.warning(f"  [MediaManager Short] Directorio vacío: {directory}")
        return None
...
def get_random_bgm(mood):
    ...
    if not bgm_path:
        logger.warning(f"  [MediaManager Short] Fallback a primera música disponible.")
...
def download_media(url, save_path, retries=2):
    ...
    logger.info(f"  [MediaManager Short] Descargando recurso: {url[:50]}...")
    ...
    for attempt in range(retries):
        try:
            ...
            logger.warning(f"  [MediaManager Short] Servidor devolvió status {response.status_code}")
            
        except Exception as e:
            logger.warning(f"  [MediaManager Short] Fallo intento {attempt+1}: {e}")
            
    logger.error("  [MediaManager Short] Fallo definitivo al descargar medio.")
    return None
