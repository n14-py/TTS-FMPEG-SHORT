...
"""
==============================================================================
TTS ENGINE (Motor de Texto a Voz - VERSIÓN SHORTS)
==============================================================================
Convierte el texto del guion en archivos de audio MP3 usando Microsoft Edge TTS.
Soporta múltiples voces y limpia el texto para evitar bloqueos del sintetizador.
Adaptado con logs específicos para la trazabilidad en formato vertical.
"""
...
async def _async_generate_audio(text, voice_code, output_path):
    ...
    try:
        ...
        logger.error(f"  [TTS Engine Short] Fallo interno en Edge TTS: {e}")
        return False
...
def generate_audio_clip(text, voice_key, filename):
    ...
    """
    logger.info(f"  [TTS Engine Short] Preparando locución para: {filename}")
    ...
    if os.path.exists(output_path):
        try:
            ...
            logger.warning(f"  [TTS Engine Short] No se pudo sobrescribir {output_path}")
    ...
    for attempt in range(max_retries):
        try:
            ...
            if success and os.path.exists(output_path) and os.path.getsize(output_path) > 100:
                logger.info(f"  [TTS Engine Short] Éxito: {filename} generado correctamente.")
                return output_path
            else:
                logger.warning(f"  [TTS Engine Short] Archivo vacío o no generado. Intento {attempt + 1}/{max_retries}")
                
        except Exception as e:
            logger.warning(f"  [TTS Engine Short] Error en intento {attempt + 1}: {e}")
            
        # Pequeña pausa antes de reintentar si el servidor de Microsoft rechazó la conexión
        import time
        time.sleep(1.5)
        
    logger.error(f"  [TTS Engine Short] ERROR FATAL: No se pudo generar audio para {filename} tras {max_retries} intentos.")
    return None
    ...
    for i, scene in enumerate(scenes_data):
        ...
        if text.strip():
            ...
            if path:
                ...
                logger.error(f"  [TTS Engine Short] Falló la escena {i}, saltando audio.")
                audio_paths[i] = None
        else:
            audio_paths[i] = None
